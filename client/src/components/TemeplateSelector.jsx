            ...
            {isOpen && (
                <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                    {templates.map((template) => (
                        <div className={`relative p-3 cursor-pointer transition-all border rounded-md ${selectedTemplate === template.id ? "border-blue-400 bg-blue-100" : 'border-gray-300 hover:border-gray-400 hover:bg-gray-100'}`} key={template.id} onClick={() => { onChange(template.id); setIsOpen(false) }}>
