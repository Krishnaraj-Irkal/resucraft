            ...
            <div className=" mt-6 space-y-6">
                {data.map((project, index) => (
                    <div key={index} className="p-4 border border-gray-300 rounded-lg space-y-3">
                        ...
                        <textarea rows={8} type="text" value={project.description || " "} onChange={(e) => updateProject(index, "description", e.target.value)} placeholder='Description your project...' className='w-full px-3 py-2 text-sm rounded-lg resize-none' />
                    </div>
                ))}
            </div>
