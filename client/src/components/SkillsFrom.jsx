  ...
  return (
    <div className='space-y-6'>
        ...
        <div className="flex gap-2">
            ...
            <button onClick={addSkills} disabled={!newSkills.trim()} className="flex items-center gap-2 px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bh-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {data.length > 0 ? (
            <div className="flex flex-wrap gap-2">
                {data.map((skill,index) => (
                    <span key={index} className="flex items-centergap-1 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                        {skill}
